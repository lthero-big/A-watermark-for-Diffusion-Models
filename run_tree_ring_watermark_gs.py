...
def init_gs_Z_s_T():
    ...
    # 步骤1：生成256bit的随机水印消息k
    k = os.urandom(32)  # 32 bytes * 8 bits/byte = 256 bits
    print("=============k===============")
    print(k)
    print("=============k===============")
    # 步骤2：扩散过程，复制64份
    ...
    # 步骤3：使用ChaCha20加密
    key = os.urandom(32)  # 生成一个256bit的随机密钥
    nonce = os.urandom(16)  # 生成一个随机nonce
    ...
    for i in range(0, len(m_bits), l):
        ...
        index+=1
        # 提取i
        i_extracted = np.floor(2**l * norm.cdf(z_s_T))

        # 存储结果
        results.append((y, i_extracted, z_s_T))

    # print(len(Z_s_T_array),len(Z_s_T_array[0]),len(Z_s_T_array[0][0]),Z_s_T_array[0][0][0])
    return Z_s_T_array
...
def main(args):
    ...
    for i in tqdm(range(args.start, args.end)):
        ...
        Z_s_T_arrays = [init_gs_Z_s_T() for _ in range(1)]

        # 将numpy数组转换为torch张量，并堆叠它们形成start_code
        init_latents_w = torch.stack([torch.tensor(Z_s_T_array).float() for Z_s_T_array in Z_s_T_arrays]).to(device)
